...
use CosmeticMenu\TaskCosmeticMenuSetItem;
...
Class Main extends PluginBase implements Listener{
    ...
    public function onJoin(PlayerJoinEvent $event) {
		...
		if ($this->getServer()->getDefaultLevel()->getName() == $player->getLevel()->getName()) {
			$player = $event->getPlayer();
			$inv = $player->getInventory();
			$inv->clearAll();
			$this->getScheduler()->scheduleDelayedTask(new TaskCosmeticMenuSetItem($this, $player), 20);
		}
    }
	...
	Public function onLevelChange(EntityLevelChangeEvent $event) {
		...
		if($player instanceof Player) {
			$inv = $player->getInventory();
			if ($this->getServer()->getDefaultLevel()->getName() == $event->getOrigin()->getName()) {
				$inv->clearAll();
				$player->removeAllEffects();
			}
			elseif ($this->getServer()->getDefaultLevel()->getName() == $event->getTarget()->getName()) {
				$inv->clearAll();
				$player->removeAllEffects();
				$this->getScheduler()->scheduleDelayedTask(new TaskCosmeticMenuSetItem($this, $player), 20);	
			}
	...
	if ($this->getServer()->getDefaultLevel()->getName() == $player->getLevel()->getName()) {
        ...
        if ($player instanceof Player) {
            ...
            $particle3 = new WaterDripParticle($pos, 12);
...
//Particles
    if($item->getId() == Item::DYE) { // Dye
        switch($item->getDamage()) {
			...
			$particle = new WaterDripParticle(new Vector3($player->x, $player->y + 2, $player->z));
			...
			$particle = new FlameParticle(new Vector3($player->x, $player->y + 2, $player->z));
			    $random = new Random((int) (microtime(true) * 1000) + mt_rand());
					for($i = 0; $i <90; ++$i){
						$particle->setComponents(
						$player->x - 2 + $random->nextSignedFloat() * $player->x+ 2,
    ...
    if($item->getId() == Item::BLAZE_ROD){
		...
		// $light->y = $player->y;
		// $light->z = $player->z;
		$light->position = $player->asVector3();
